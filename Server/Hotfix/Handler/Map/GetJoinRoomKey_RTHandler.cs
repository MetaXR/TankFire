...
namespace Hotfix
{
    ...
    {
        protected override Task Run(Room room, GetJoinRoomKey_RT message, Action<GetJoinRoomKey_RE> reply)
        {
            ...
            try
            {
                ...
                Unit unit = EntityFactory.CreateWithId<Unit,long>(message.PlayerID, message.UserID);               
                gamer.unit = unit;
                //随机密匙
                long key = RandomHelper.RandInt64();
                room.GetComponent<RoomJoinKeyComponent>().Add(key, gamer);
