    ...
    public class Login_RTHandler : AMRpcHandler<Login_RT, Login_RE>
    {
        protected override async void Run(Session session, Login_RT message, Action<Login_RE> reply)
        {
            ...
            try
            {
                ...
                StartConfig gateConfig = Game.Scene.GetComponent<StartConfigComponent>().Get(5);
                Session gateSession = Game.Scene.GetComponent<NetInnerComponent>().Create(NetworkHelper.ToIPEndPoint(gateConfig.GetComponent<OuterConfig>().Address2));
                Log.Info(gateConfig.GetComponent<OuterConfig>().Address2);
                //请求登录Gate服务器密匙
