...
public class RtsCamera : MonoBehaviour
{
    ...
    /// </summary>
    /// 
    public void SetCameraValue(GameObject go)
    {
        if (go.name == "dakua")
        {
           //LookAt = new Vector3(0,0,0);
            Distance = 1000;
            MinDistance = 300;
            MinBounds = new Vector3(-1000f, -1000f, -1000f);
            MaxBounds = new Vector3(1000f, 1000f, 1000f);
        }
    }
    ...
    private void CreateTarget()
    {
        ...
        _target.GetComponent<Renderer>().material.color = new Color(0,1,0,0.5f) ;
        ...
        _target.transform.position = LookAt;
        _target.AddComponent<CameraTargetSetSelf>();//添加一个控制脚本
        isFirst = false;
    }
